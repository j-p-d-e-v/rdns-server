    ...
    pub fn read(buffer: &mut BytePacketBuffer) -> Result<DnsRecord,String> {
        ...
        match qtype {
            ...
            QueryType::CNAME => {
                ...
                Ok(DnsRecord::CNAME { domain: domain, host: cname, ttl: ttl })
            }
            QueryType::NS => {                
                let mut ns = String::new();
                buffer.read_qname(&mut ns)?;
                Ok(DnsRecord::NS { domain: domain, host: ns, ttl: ttl })
            }
            QueryType::MX => {
                let priority = buffer.read_u16()?;
                let mut mx = String::new();
                buffer.read_qname(&mut mx)?;                
                Ok(DnsRecord::MX { domain: domain, priority: priority, host: mx, ttl: ttl })
            }
            QueryType::UNKNOWN(_) => {
